...
public class TestWorld extends ActorWorld
{
  ...
  private Location loc = (0, 0);//creating a new location
  ...
  {
    loc = display.getCurrentLocation();//get the location at the space clicked
    setMessage("");
    ...
    if(alreadyClicked)//if the player has selected a valid piece
    {
      if(!pieceIsKing)//not a king
      {
        ...
        if(moveDirection == JOptionPane.YES_OPTION)
        {
          boolean movable = validStep(int 1, loc);//call on private method validStep
        ...
        else if(moveDirection == JOptionPane.NO_OPTION)
        {
          boolean movable = validStep(int 2, loc);//call on private method validStep
      ...
      if(pieceIsKing)
      {
        ...
        if(moveLeftRight == JOptionPane.YES_OPTION)
        {
          ...
          if(moveUpDown == JOptionPane.YES_OPTION)
          {
            boolean movable = validStep(int 3, loc);//call on private method validStep
          ...
          if(moveUpDown == JOptionPane.NO_OPTION)
          {
            boolean movable = validStep(int 4, loc);//call on private method validStep
